...
Phase A of the Family Menu Action Assistant is also deterministic and browser-first. Filling empty dinners and refreshing the shopping list reuse existing ranking and grocery helpers; they do not call OpenAI. Typed requests currently map onto those same chips with keyword matching in the browser (Phase B lite), plus read-only this-week/next-week dinner lookups for “what’s for” questions; they do not call a model. A later typed-request phase may use the model only after household validation and the existing AI usage caps, and only to rank or interpret a request. The assistant must still preview and require Apply before writes; lookup answers must not write. It must never silent-write a plan.
