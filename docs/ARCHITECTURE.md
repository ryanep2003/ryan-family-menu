...
The optional weekly dinner draft uses published household recipes and stays in browser memory until approved. Approval reads the latest schedule, checks each selected date against the draft's effective-meal baseline, and sends one versioned schedule write. A `409` permits at most one fresh recheck; a same-date change requires review rather than entering the normal schedule auto-merge path. Shopping remains a separate manual review and is not changed by draft approval.

After a confirmed draft approval, the Plan screen reads fresh schedule and grocery records to preview this week's shopping changes. `previewPlannedGroceryChanges()` flags checked rows whose amount or use changed or whose planned contribution disappeared. A separate explicit update re-reads both records and requires the previewed versions. It blocks any change that would alter or remove checked purchase evidence, then sends one versioned grocery write; a `409` requires a fresh preview. Manual rows and matching planned-row IDs survive the rebuild. Schedule and groceries remain separate writes, so the UI reports their outcomes independently.

The older Shop build action and approved school-lunch grocery sync also check whether a proposed rebuild would change or remove checked purchase evidence before changing the browser list. If so, they stop and ask for review. Lunch approval reports separately when the lunch plan saved but its shopping update did not. These older save paths still use their existing grocery conflict merge after a local rebuild; they are not a complete purchase-reconciliation workflow.

`change-of-plans-logic.js` provides dinner attendance/batch previews, per-recipe scaled ingredient contributions, and conservative known-time dinner alternatives. Today can save an attendance adjustment or replace today's dinner main with a reviewed known quick recipe through `change-of-plans-client.js`: both read the latest versioned schedule, compare the exact date with the preview baseline, and make one date-specific write. A same-date change or `409` stops without an automatic overwrite. Immediate undo uses the saved server meal as its baseline and repeats the fresh-date check. The quick swap keeps the existing dinner item ID, sides, other meals, and serving plan; recipes needing restriction review stay in the manual Plan editor. Neither preview nor save writes groceries. A missing recipe or unquantified ingredient is reported as unknown rather than presented as a precise shopping amount.
...
- `schedule-utils.js` and `schedule-ui.js`: canonical meals, legacy compatibility, portions, leftovers, next-week calendar persist, the Plan save bar, and the local pending plan reconciled on reload. `app.js` restores each bottom tab’s scroll position when that tab is opened again.
