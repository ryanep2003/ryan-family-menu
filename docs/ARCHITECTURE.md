...
- `dinner-flow.js`: Today open-dinner samples, list-first choose-dinner filtering, exact recipe-id selection for list → review → confirm, and serving-count field rewrite helpers used by meal review. Missing recipe ids resolve to null and must not fall back to another catalog recipe.
