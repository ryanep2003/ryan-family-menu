...
- A Today view for tonight’s dinner. When dinner is still open, the hero asks what sounds good and offers one Choose dinner action. Choosing a recipe opens a short Favorites-and-recent strip above a photo grid, with Grid | List for that browse area, search, All / Favorites / Sides filters, and a fixed decision tray. Tapping a card selects it. Make it a meal review covers eaters, extra portions, and an optional side. Confirm writes the dinner through the existing meal save; it never saves from the picker. Plan’s add-to-meal search uses the same strip and grid, and a tap there still adds the recipe. Breakfast/lunch stay under Also today, with Plan & shop shortcuts, Help, handoff, and household tools remaining available without sitting in the daily loop.
- Four primary destinations: Today, Plan, Shop, and Library. School Lunches stays in the product and opens from Plan.
- Weekly and monthly planning across breakfast, lunch, and dinner. Plan keeps a navy Save bar on screen while a meal change is unsaved or still waiting to sync, then shows a quiet saved confirmation on Plan after the schedule record accepts it. Meal add/remove/edit auto-saves. An unsaved plan stays on this phone until that write succeeds, so a reload does not drop it.
...
- A shared household recipe library backed by the platform catalog, local drafts, editing, favorites, and hiding/deletion overlays. Search is at the top of the library. On phones, a short Favorites-and-recently-cooked strip sits above a two-column photo grid of the filtered set; Grid | List can switch the main browse layout, and a recipe can be added directly to a day and meal from the recipe screen. The platform catalog is the single source for starter and family recipes; local cache supports brief offline recovery.
