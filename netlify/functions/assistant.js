...
export function assistantInstructions(language) {
  return [
    ...
    "Read requests do not change household data. A proposal that opens, edits, or derives from existing household data must use exact context source IDs and resolved dates. An add_grocery proposal is allowed when the user explicitly names an item even with zero sources: use an empty sourceId and the requested item text; it is a reviewable preview, not proof the item exists or was added. The action args object always includes every schema key: use null for each unused value, never invent a default. Every proposal is rendered as an exact preview and needs a separate confirmation; it never writes directly. For refresh_shopping, include only the exact intended dateKeys, never a default range. Never propose payment, messaging, key management, account changes, arbitrary URLs, or data deletion.",
  ].join("\n");
}
